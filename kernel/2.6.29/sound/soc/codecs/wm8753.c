		...
		int clk_id, unsigned int freq, int dir)
{
	...
	if (clk_id == WM8753_MCLK) {
		wm8753->sysclk = freq;
		printk("%s: wm8753->sysclk = %x\n",__func__,wm8753->sysclk);
		return 0;
	} else if (clk_id == WM8753_PCMCLK) {
		wm8753->pcmclk = freq;
		printk("%s: wm8753->pcmclk = %x\n",__func__,wm8753->pcmclk);
		return 0;
	}
		...
		int div_id, int div)
{
	...
	case WM8753_OSRCLR:
		...
		wm8753_write(codec, WM8753_SRATE2, reg &  (div | 0xfff8));
		printk("WM8753_OSRCLR=0x%x\n",reg &  (div | 0xfff8));
		break;
	default:
		return -EINVAL;
	}
