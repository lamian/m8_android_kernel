...
s3c_serial_rx_chars(int irq, void *dev_id)
{
 ...
 out:
	 wake_lock_timeout(&uart_wakelock, HZ);
	return IRQ_HANDLED;
}
...
#ifdef M8_SERIAL
/*
 * this feature is for m8 only.
 * When CTS is deactivated, invalidate RTS for a short while (typically less than 500ms) to wake peer up
 */
static void
s3c_serial_wake_peer(struct uart_port *port)
{
	int gpadata, i;
	
	if (port->line != 0) return;

	for (i=0; i<5; i++) {
		if (__raw_readl(S3C64XX_GPADAT) & (1<<2)) {
			__raw_writel(__raw_readl(S3C64XX_GPADAT) | (1<<3), S3C64XX_GPADAT);
			msleep(1);
			__raw_writel(__raw_readl(S3C64XX_GPADAT) & (~(1<<3)), S3C64XX_GPADAT);
			msleep(100);
		} else {
	...
	printk("Modem is not responding to signals!!\n");
}
#endif
...
#ifdef M8_SERIAL
	.wake_peer  = s3c_serial_wake_peer,
#endif
};
...
{
	wake_lock_destroy(&uart_wakelock);
