...
static int spu_set_bus_mode(struct if_spi_card *card, u16 mode)
{
	...
	if (err)
		return err;
	if (rval != mode) {
		lbs_pr_err("Can't read bus mode register. mode=%d, rval=%d\n", mode, rval);
		return -EIO;
	}
	return 0;
}
			      ...
			      char *helper_fw, char *main_fw)
{
	...
	snprintf(helper_fw, IF_SPI_FW_NAME_MAX, "gspi%d_helper.bin",
...
int if_spi_suspend(struct spi_device *spi, pm_message_t mesg)
{
	printk("Entering if_spi_suspend");
	struct if_spi_card *card = spi_get_drvdata(spi);
	struct lbs_private *priv = card->priv;
	lbs_suspend(priv);
	return 0;
...
int if_spi_resume(struct spi_device *spi)
{
	printk("Entering if_spi_resume");
	struct if_spi_card *card = spi_get_drvdata(spi);
	struct lbs_private *priv = card->priv;
	lbs_resume(priv);
	return 0;
