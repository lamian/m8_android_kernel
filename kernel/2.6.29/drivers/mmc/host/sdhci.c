...
static void sdhci_cmd_irq(struct sdhci_host *host, u32 intmask)
{
	...
	if (!host->cmd) {
		printk("%s: Got command interrupt 0x%08x even "
	...
	if (host->cmd->flags & MMC_RSP_BUSY) {
		...
		present = readl(host->ioaddr + SDHCI_PRESENT_STATE);
		printk("busy? present %08x, intstat %08x\n", present, intmask);
...
static void sdhci_data_irq(struct sdhci_host *host, u32 intmask)
{
	...
	if (!host->data) {
		...
		printk("%s: Got data interrupt 0x%08x even "
...
int sdhci_add_host(struct sdhci_host *host, unsigned int hostcaps)
{
	struct mmc_host *mmc;
	unsigned int caps;
	int ret;
